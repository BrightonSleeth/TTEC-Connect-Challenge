## Misc
- Completely serverless. Nothing to maintain.
- Connect is the entry point. Contact flow routes to lambda, waits for a response, then repeats in tts the best 3 candidates for the callers phone number.
...
- A simple HTTP API Gateway linked to lambda is the entry point for the dashboard. I opted for HTTP instead of REST since for our use case we dont need any of the extra features REST offers, and lowers costs.
